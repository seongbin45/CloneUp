...
| **첫 실행 안내** | **적용** (`onboarding_dialog.py`, 7단계) | 예전 표의 「3단계 위저드 보류」는 폐기. 전체화면 모달. 건너뛰기·도움말에서 재개 가능. 번복 사유: 미기록 → 이후 구현으로 확정 |
| **자리 모델** | **네 자리** (작업 폴더 · staging · 커밋 · GitHub) | 제품이 커밋 단위로만 remote에 맞춤. 온보딩·용어·팁 통일 |
| **로그인 방식** | **PAT 기본** (Device Flow = `CLONEUP_ALLOW_DEVICE_FLOW=1` 만) | 공개 client_id 남용 회피. 난이도↑는 인정, 연결 마법사로 완화 |
