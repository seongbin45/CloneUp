...
|------|------|
| 원본 시안 HTML | 있음 |
| 원본 비트맵 | `desin/icon/png/CloneUp Logo-selection*.png` 로 제공됨 (2026-08-06) |
| 현재 `assets/icons/*` | **시안 크기 스트립에서 추출** (`scripts/import_design_pngs.py`) |
| 창 아이콘 연결 (I3) | 원본 마크(계단+화살표) 적용 |

이전 플레이스홀더(문서 2장+화살표)는 폐기. 심볼은 **올라가는 계단 + 화살표**.
...
- Git 최초 아이콘 커밋(생성 스크립트 산출물) — 전부 플레이스홀더 계열
