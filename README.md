...
| 항목 | 내용 |
|------|------|
| 현재 버전 | **0.1.10** (`VERSION`, `app/__init__.py`, `installer/CloneUp.iss`) |
...
클론업은 초심자가 Git을 쓰는 **이유**를  
탭과 안내에 남깁니다.

1. **커밋** — 작업 폴더 내용을 로컬 커밋(commit)으로 남김  
2. **올리기** — 그 기록을 GitHub(remote)에도 맞춤 (`git push`)  
3. **받아오기** — 원격의 새 기록을 이 PC에 맞춤 (`git pull`)  
4. **다시 수정** — 작업 폴더에서 고친 뒤 다시 커밋  

「충돌 취소」는 받아오기 중 **겹침을 포기**하는 비상 버튼이고,  
「커밋 내역」은 **지난 작업 지점**을 보거나 되돌리는 도구입니다 (서로 다릅니다).  
...
- 줄: `#define MyAppVersion "0.1.8"`  
→ 예: `0.1.8` (버그 수정), `0.2.0` (기능 추가)
