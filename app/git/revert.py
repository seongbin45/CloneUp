...
@dataclass(frozen=True)
class HardRevertResult:
    """
    History-rewriting revert: HEAD *becomes* target_full_hash — no new commit.

    Anything between the old HEAD and the target is no longer reachable from
    this branch (only from ``backup_branch``); a force push carries that
    forward to origin.
    """
...
def hard_revert_local_commit(
    folder: str | Path,
    target_rev: str,
) -> HardRevertResult:
    """
    Local half of a hard revert: backup branch → ``reset --hard`` target.

    No network access — split out from ``hard_revert_to_commit`` so it can be
    unit-tested without a GitHub remote. Push is the caller's job.
    """
    root = _ensure_git_repo(Path(folder))
    ...
    try:
        run_git(["reset", "--hard", target_full], cwd=str(root), check=True)
    except GitError as e:
        raise RevertError(f"되돌리기 실패: {e}") from e

    return HardRevertResult(
        folder=root,
        target_full_hash=target_full,
        target_short_hash=target_short,
        target_message=target_message,
...
def hard_revert_to_commit(
    ...
    *,
    ...
    """
    Hard-revert *folder* to *target_rev* (reset --hard) and force-push.

    Requires a clean working tree first. Unlike ``revert_to_commit``, this
    rewrites the branch itself — a backup branch is still made, but the push
    uses ``--force-with-lease`` because origin's history is being replaced,
    not extended.
    """
    root = _ensure_git_repo(Path(folder))
    try:
    ...
    result = hard_revert_local_commit(root, target_rev)
    ...
    try:
        ...
        if r.returncode != 0:
            ...
            raise RevertError(
                "이 컴퓨터의 기록은 되돌렸지만 GitHub로 보내지 못했습니다.\n"
                f"백업 브랜치: {result.backup_branch}\n"
                "GitHub에 그 사이 다른 변경이 올라왔을 수 있습니다 "
                "(다른 컴퓨터·동료가 올린 커밋 등). 확인 후 다시 시도하세요."
                + (f"\n\n(참고)\n{out[:400]}" if out else "")
            )
    finally:
        delete_credential_file(cred_path)
