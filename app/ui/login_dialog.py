...
class ConnectGitHubWizard(QDialog):
    ...
    def __init__(
        ...
        *,
        ...
        # Always NonModal: GitHub/Apple passkey, Windows Security, and other
        # OS/provider popups must be able to appear *above* this dialog.
        # ApplicationModal previously covered those sheets (PIN unreachable).
        self.setWindowModality(Qt.WindowModality.NonModal)
        self.setWindowOpacity(1.0)
        # Normal chrome: title bar + close (X). Never frameless / FullScreen.
        # Do not use WindowStaysOnTopHint — other apps (browser / OS UI) must
        # be able to come in front of this dialog.
        # Always include Maximize in the initial flags. Toggling
        # WindowMaximizeButtonHint via setWindowFlag() while this dialog is
        # in exec() **hides** the window on Windows and ends exec() as
        # Rejected — main then logs 「연결 안내 취소」 even though WebView
        # may still issue a PAT.
        ...
        self._choice_geometry_locked = False
        self._choice_fit_gen = 0
        self._normal_fit_gen = 0
        self._normal_fit_expected = 0
        # Path A: keep OS passkey / Hello sheet in front when it appears.
        self._passkey_fg_active = False
    ...
    def showEvent(self, event) -> None:  # noqa: N802
        ...
        if self._use_web and self._on_flow_narrow_page():
            # One delayed settle pass only — dual 0+50 timers + resize clamps
            # flooded the queue on dpr=2 (white / Not Responding).
            if not self._choice_geometry_locked:
                self._schedule_fit_choice_dialog(0)
        self._place_away_banner()

    def resizeEvent(self, event) -> None:  # noqa: N802
        super().resizeEvent(event)
        # Do NOT clamp choice size here. On Windows high-DPI, setGeometry/
        # resize often land off the requested client size; clamping re-posted
        # resize events forever → white dialog + Not Responding (Ctrl+C).
        self._place_away_banner()
    ...
    def _set_maximize_button(self, enabled: bool) -> None:
        """No-op for window flags while ``exec()`` is running.

        Historically this called ``setWindowFlag(Maximize…)`` when entering
        WebView. On Windows that can hide the dialog and finish ``exec()`` as
        ``Rejected`` (main log: 연결 안내 취소) even after Generate succeeded.
    ...
    def _schedule_fit_choice_dialog(self, delay_ms: int = 0) -> None:
        ...
        if getattr(self, "_choice_geometry_locked", False):
            return
    ...
    def _unlock_choice_geometry(self) -> None:
        """Allow one new fit after intro↔choice navigation."""
        self._choice_geometry_locked = False
    ...
    def _fit_choice_dialog(self) -> None:
        ...
        After one successful fit, ``_choice_geometry_locked`` prevents further
        setGeometry / clamp storms on high-DPI Windows.
        """
        ...
        try:
            ...
            # Size from the flow *card* sizeHint only — do NOT adjustSize()
            # (Expanding stack + adjustSize cascaded layouts on dpr=2).
            idx = self._stack.currentIndex()
            ...
            # Stable shell: choice cards are roughly fixed; chasing a tall
            # sizeHint (→600) fought Windows mintrack and starved paint.
            if idx == getattr(self, "_choice_index", -1):
                pw = max(pw, 500)
                phh = min(max(phh, 520), 560)
            ...
            # Soft minimum only — never setMaximumSize on choice (DPI fight).
            ...
            try:
                ...
                nw, nh = w, h
            # Record whatever Windows applied; do not keep resizing toward `w,h`.
            self._choice_size = (max(1, nw), max(1, nh))
            self._choice_geometry_locked = True
            try:
    ...
    def _poll_passkey_os_prompt(self) -> None:
        """If OS passkey / Hello UI is open, keep it above this dialog."""
        ...
        try:
            ...
            hwnd = 0
        if hwnd:
            self._ensure_passkey_os_in_front(force=False)
        elif self._passkey_fg_active:
            self._passkey_fg_active = False
            self._wiz_log("[연결] 패스키 OS 창 종료 감지")

    def _ensure_passkey_os_in_front(self, *, force: bool = False) -> None:
        """
        Raise Windows Security / Apple / Hello passkey sheet above us.

        Dialog stays NonModal always (see ``__init__``). We only nudge the
        OS HWND forward — no modality toggle, no dimming.
        """
        from app.util.browser_address import bring_passkey_os_prompt_forward

        ok, detail = bring_passkey_os_prompt_forward()
        if ok:
            if force or not self._passkey_fg_active:
                self._wiz_log(f"[연결] 패스키 창 앞으로: {detail}")
            self._passkey_fg_active = True
        elif force:
            self._wiz_log(f"[연결] 패스키 창 대기: {detail}")
        self._start_passkey_os_poll()
    ...
    def changeEvent(self, event) -> None:  # noqa: N802
        ...
        if self._on_flow_narrow_page():
            ...
            if became_max or became_fs:
                self._suppress_state_fit = True
                self.setWindowState(Qt.WindowState.WindowNoState)
                # If geometry already settled, only un-maximize — re-fitting
                # here re-entered setGeometry storms on high-DPI displays.
                if not getattr(self, "_choice_geometry_locked", False):
                    self._schedule_fit_choice_dialog(0)
            return
    ...
    def _go(self, index: int) -> None:
        if self._use_web:
            ...
            if index == intro_i:
                ...
                clear_size_locks(self)
                self.setMinimumSize(640, 360)
            prev_i = self._stack.currentIndex()
            self._stack.setCurrentIndex(index)
            self.setWindowOpacity(1.0)
            if index in (intro_i, choice_i):
                # New narrow page (or first show) → one fit; stay locked after.
                if prev_i != index or not self._choice_geometry_locked:
                    self._unlock_choice_geometry()
                    self._schedule_fit_choice_dialog(0)
            else:
    ...
    def _on_web_stage(self, stage: object) -> None:
        ...
        if st == GitHubPageStage.AUTH_PASSKEY_OS:
            self._ensure_passkey_os_in_front(force=True)
    ...
    def _on_webview_flow_classified(
        ...
        if kind == "current" and idx is not None:
            ...
            if method == "passkey":
                # WebAuthn / Use passkey — bring OS sheet above us if present.
                self._ensure_passkey_os_in_front(force=True)
            try:
                i = int(idx)
